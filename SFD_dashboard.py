...
.section-header {
    ...
    font-size: 0.9rem; color: #00c6ff; text-transform: uppercase;
    letter-spacing: 3px; border-left: 3px solid #00c6ff;
...
COOLDOWN         = 30
WARNING_WINDOW   = 30
FLARE_WINDOW_MIN = 15   # ±minutes around a flare peak to label a row as y_true=1
...
# (previously generate_model_evaluation_report.py)
# Runs once on startup; writes Model_Eval_Results.json + 8 PNGs.
# ==========================================================

def run_model_evaluation():
    """
    Compute row-level classifier metrics and save:
      - Model_Eval_Results.json  (loaded by the dashboard below)
      - outputs/01_Forecast_Probability.png  … 08_ROC_Curve.png
    Returns True on success, False if required files are missing.
    """
    pred_path  = os.path.join(SCRIPT_DIR, "Prediction_Test_Period.csv")
    flare_path = os.path.join(SCRIPT_DIR, "Unified_Flare_Catalog.csv")
    if not os.path.exists(pred_path) or not os.path.exists(flare_path):
        return False
    ...
    with open(os.path.join(SCRIPT_DIR, "Model_Eval_Results.json"), "w") as f:
...
# ==========================================================
# RUN EVALUATION ON STARTUP (cached so it only runs once)
# ==========================================================

@st.cache_resource
def ensure_model_eval():
    json_path = os.path.join(SCRIPT_DIR, "Model_Eval_Results.json")
    # Re-run if JSON is missing OR if predictions CSV is newer than JSON
    pred_path = os.path.join(SCRIPT_DIR, "Prediction_Test_Period.csv")
    needs_run = not os.path.exists(json_path)
    if not needs_run and os.path.exists(pred_path):
        needs_run = os.path.getmtime(pred_path) > os.path.getmtime(json_path)
    if needs_run:
...
@st.cache_data
def load_data():
    pred  = pd.read_csv(os.path.join(SCRIPT_DIR, "Prediction_Test_Period.csv"))
...
# ==========================================================
# OPERATIONAL WARNING-LEVEL METRICS
# ==========================================================
...
op_recall    = op_tp/(op_tp+op_fn) if (op_tp+op_fn) > 0 else 0.0
op_f1        = 2*op_precision*op_recall/(op_precision+op_recall) if (op_precision+op_recall) > 0 else 0.0

# ==========================================================
# MODEL-LEVEL EVALUATION (loaded from JSON)
# ==========================================================

model_eval = None
eval_path  = os.path.join(SCRIPT_DIR, "Model_Eval_Results.json")
...
if model_eval is not None:
    ...
    st.sidebar.warning("Model_Eval_Results.json not found — ensure Prediction_Test_Period.csv exists so it can be generated automatically.")

st.sidebar.markdown("## ⚠️ OPERATIONAL WARNING PERFORMANCE")
st.sidebar.caption("After threshold + persistence + cooldown post-processing")
st.sidebar.markdown(f"""
<div class="sidebar-badge">
<p>✅ Precision: <span>{op_precision:.3f}</span></p>
<p>🔁 Recall: <span>{op_recall:.3f}</span></p>
<p>⚖️ F1 Score: <span>{op_f1:.3f}</span></p>
<p>🟢 True Positive Warnings: <span>{op_tp}</span></p>
<p>🔴 False Positive Warnings: <span>{op_fp}</span></p>
<p>⚠️ Missed Flares: <span>{op_fn}</span></p>
</div>""", unsafe_allow_html=True)
...
with c3:
    total_obs = op_tp + op_fn if (op_tp+op_fn) > 0 else 0
    st.markdown(f"""<div class="metric-card c-green">
        <div class="metric-label">Detected Flares</div>
        <div class="metric-value">{op_tp}<span style="font-size:1rem;color:#3a7a9c"> / {total_obs}</span></div>
        <div class="metric-sub">true positive warnings</div>
    </div>""", unsafe_allow_html=True)
with c4:
...
with tab3:
    a1, a2 = st.columns(2)
    with a1:
        st.markdown('<div class="section-header" style="font-size:0.75rem;color:#ffd166;border-left-color:#ffd166;">PREDICTION SUMMARY (WARNINGS)</div>', unsafe_allow_html=True)
        st.markdown(f"""
...
<p style="color:#7fb8d8;margin:0.3rem 0">True Positive Warnings: <span style="color:#00e676">{op_tp}</span></p>
<p style="color:#7fb8d8;margin:0.3rem 0">False Positive Warnings: <span style="color:#ff6b35">{op_fp}</span></p>
<p style="color:#7fb8d8;margin:0.3rem 0">Missed Flares: <span style="color:#ff3c00">{op_fn}</span></p>
</div>""", unsafe_allow_html=True)
    ...
    with a2:
        ...
        if model_eval is not None:
            ...
            st.warning("Model evaluation data unavailable. Ensure Prediction_Test_Period.csv exists and reload.")
...
with tab5:
    if model_eval is not None and roc_fpr is not None:
        ...
        st.warning("ROC data unavailable. Ensure Prediction_Test_Period.csv exists and reload.")
...
with tab6:
    ...
    if st.button("📄 Generate Summary Report ↗"):
        ...
        model_block = (
            ...
            "- Not available. Ensure Prediction_Test_Period.csv exists and reload."
        )
...
## Operational Warning Performance (threshold + persistence + cooldown)
- Precision:  {op_precision:.3f}
- Recall:     {op_recall:.3f}
- F1 Score:   {op_f1:.3f}
- True Positive Warnings:  {op_tp}
- False Positive Warnings: {op_fp}
- Missed Flares:   {op_fn}
